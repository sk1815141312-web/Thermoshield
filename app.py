...
def inject_custom_css():
    st.markdown(
        ...
        Ask ThermoShield chat box) needs its own dark color here — these
        boxes keep a light background even in the dark theme, so without
        this override the text inherits the page's near-white color and
        becomes invisible while typing. */
        ...
        .thermo-loader-wrapper {
            ...
            margin: 20px auto;
            max-width: 420px;
            background: linear-gradient(145deg, #0F1B2B 0%, #0B0F19 100%);
        ...
        .thermo-loader-ring {
            ...
            animation: thermoSpin 1.1s linear infinite;
            margin-bottom: 18px;
        ...
        @media (max-width: 640px) {
            ...
            .thermo-loader-ring { width: 76px; height: 76px; }
        }
        </style>
        """,
...
def thermo_loading(status_text: str):
    """
    Context manager that shows a big animated ThermoShield loading ring while
    a block of code runs (API calls etc.), then clears it automatically —
    replaces the plain st.spinner text with a branded, professional-looking
    indicator. Usage: `with thermo_loading("Fetching routes..."): ...`
    """
    placeholder = st.empty()
    placeholder.markdown(
        f"""
        <div class="thermo-loader-wrapper">
            <div class="thermo-loader-ring"></div>
            <div class="thermo-loader-text">⚡ {status_text}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def build_map(origin, destination, route_options, selected_name):
    ...
    for opt in route_options:
        ...
        folium.PolyLine(
            ...
            tooltip=(
                f"{label} | {opt['distance_km']} km | {opt['duration_min']:.0f} min | "
...
def generate_gemini_advisory(route_options, selected_name, api_key):
    ...
    for r in route_options:
        ...
        fact_lines.append(
            f"- {r['raw_name']}: distance {r['distance_km']} km, ETA {r['duration_min']:.0f} min, "
...
def ask_thermoshield(question, route_options, selected_name, weather_data, advisory_data):
    ...
    selected = next(r for r in route_options if r["raw_name"] == selected_name)
    facts_lines = [
        f"- {r['raw_name']}: {r['distance_km']} km, {r['duration_min']:.0f} min ETA, "
    ...
    if matches(safe_words):
        ...
        return (
            ...
            f"{coolest['duration_min']:.0f} min."
        ), "Rule-Based Engine"

    if matches(fast_words):
        fastest = min(route_options, key=lambda r: r["duration_min"])
        return (
            f"The fastest option is **{fastest['name']}** at {fastest['duration_min']:.0f} min "
    ...
    if matches(distance_words):
        return (
            ...
            f"(ETA {selected['duration_min']:.0f} min)."
        ), "Rule-Based Engine"

    if matches(summary_words):
        return (
            f"**{selected['name']}**: {selected['distance_km']} km, {selected['duration_min']:.0f} min, "
...
Live Weather Dashboard names its actual data source for the current search.
    ...
    defaults = {
        ...
        "auto_calculate": False, "chat_history": [],
    }
    ...
    if st.session_state.route_options:
                ...
                <div class="weather-card">
                    ...
                    <div style="margin-top:16px; display:flex; flex-direction:column; gap:10px;">
                        ...
                        <div>
                            ...
                            <div style="font-size:1.3rem; font-weight:700;">{selected['duration_min']:.0f} min</div>
                        </div>
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "ETA (min)": st.column_config.NumberColumn(format="%d min"),
